...
export interface GranolaSyncSettings {
  ...
  useDailyNoteFolderStructure: boolean;
}
...
export const DEFAULT_SETTINGS: GranolaSyncSettings = {
  ...
  useDailyNoteFolderStructure: false,
};
...
export class GranolaSyncSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Path to the Granola token file relative to your vault root (e.g., "configs/supabase.json"). This file contains your authentication token and needs to be copied from your Granola application directory. On macOS, it\'s typically located at ~/Library/Application Support/Granola/supabase.json. The token expires periodically, so you may need to update this file when authentication fails.'
      )
      .addText((text) =>
        text
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Enable periodic sync of notes from Granola, so you don't have to manually sync."
      )
      .addToggle((toggle) =>
        toggle
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Append synced notes to the daily note corresponding to their creation date, instead of creating separate files."
      )
      .addToggle((toggle) =>
        toggle
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'The heading to use for the section where Granola notes will be added in your daily notes. Example: "## Granola Sync"'
      )
      .addText((text) =>
        text
    ...
    // Only show this setting when not syncing to daily notes
    if (!this.plugin.settings.syncToDailyNotes) {
      new Setting(containerEl)
        .setName("Use Daily Note Folder Structure")
        .setDesc(
          "Store Granola notes using the same folder structure as your daily notes (based on your daily note format settings). When enabled, notes will be organized by date in the daily notes folder hierarchy."
        )
        .addToggle((toggle) =>
          toggle
            .setValue(this.plugin.settings.useDailyNoteFolderStructure)
            .onChange(async (value) => {
              this.plugin.settings.useDailyNoteFolderStructure = value;
              await this.plugin.saveSettings();
            })
        );
    }
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        this.plugin.settings.useDailyNoteFolderStructure
          ? "This setting is ignored when 'Use Daily Note Folder Structure' is enabled."
          : "Folder name to write notes to, if you are not syncing to daily notes and not using daily note folder structure."
      )
      .addText((text) =>
        text
